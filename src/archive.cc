...
void archiver::archive(const char *filename, bool unarc, uint8_t unarc_null){ //функция основного цикла архивирования
  ...
  pack.init();
  pack.set_operators(read_operator,write_operator);
#ifdef _WIN32
  wchar_t *wc_buf=NULL;
  HANDLE dhndl;
  FILETIME atime;
  ...
  if(unarc){
    ...
    while(tree->get_next_relative(&path,info)){
      ...
      if(S_ISDIR(info.info.attributes)){
        ...
        while(1){
          ...
          if(rw_length_file>_RC32_BUFFER_SIZE) length=_RC32_BUFFER_SIZE;
          ...
          if((length!=_RC32_BUFFER_SIZE)&&((off64_t)length!=rw_length_file)){
            errcode=0x00000004;
            break;
          };
          ...
          if(rw_length_file>_RC32_BUFFER_SIZE) rw_length_file-=_RC32_BUFFER_SIZE;
    ...
    while(tree->get_next_relative(&path,info)){
      ...
      rw_length_file=0;
      while(1){
        length=read(tm_file,content,_RC32_BUFFER_SIZE);
#ifdef EINTR
        if(length<0 && errno==EINTR) continue;
#endif
        if(length<=0) break;
